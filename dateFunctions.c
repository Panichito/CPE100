...
{
   if ((year%4 == 0 && year%100 != 0) || (year%400 == 0))  // Added year%400 == 0 
       return 1;
   else
       return 0;  
}
...
{ 
    // I have setted date 2 = date TODAY (which called in my validate function)
    int compareValue = 0;
    if (year1 > year2) 
    {
        compareValue = 1; // if built year more than year today, that means in future
    }
    else if (year1 < year2) 
    {
        compareValue = -1; // if built year less than today year, 100% in the past 
    } 
    // Same year
    ...
    /* otherwise they are the same date */
    return compareValue;
}    